...
8. **文档与代码同步**：新增或修改功能（含管理 API、UI 文案、配置字段、环境变量）时，必须同步更新 `README.md`、`README.en.md` 与 `api-test/polymock.openapi.json` 契约；审核 PR 时把「文档是否存在过时描述」作为检查项之一，禁止留下与实现脱节的文档。
